...
export default function CheckoutPage() {
  ...
  const { data: apiPaymentMethods, isLoading: isLoadingPayments } = usePaymentMethods();
  ...
  React.useEffect(() => {
    if (apiPaymentMethods && apiPaymentMethods.length > 0 && !selectedPayment) {
      const def = apiPaymentMethods.find((m) => m.is_default) ?? apiPaymentMethods[0];
  ...
  const { data: apiSlots, isLoading: isLoadingSlots } = useDeliverySlots(selectedZoneId, deliveryDate);

  const displayTimeSlots = useMemo(() => {
    if (apiSlots && apiSlots.length > 0) {
      return apiSlots.filter((s) => s.is_active).map((s) => ({
        id: s.id,
        ...
        <div className="flex-1 space-y-6">
          ...
          >
            ...
            {isLoadingAddresses ? (
              ...
              <div className="space-y-2 mb-4">
                {addresses.map((addr) => {
                  const isSelected = !showNewAddressForm && selectedAddressId === addr.id;
                  return (
                    <label
                      key={addr.id}
                      ...
                      style={
                        isSelected
                          ? { backgroundColor: 'rgba(27,67,50,0.05)', border: '2px solid #1B4332' }
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-semibold" style={{ color: '#1A1A2E' }}>
                          {addr.label}{addr.is_default && <span className="ml-2 text-xs font-normal" style={{ color: '#059669' }}>(Default)</span>}
                        </p>
                        <p className="text-xs" style={{ color: '#6B7280' }}>
                          {addr.line_1}{addr.line_2 ? `, ${addr.line_2}` : ''}, {addr.city}, {addr.province} {addr.postal_code}
                        </p>
                      </div>
                    </label>
                  );
                ...
                <button
                  ...
                  style={{ color: '#1B4332' }}
                >
                  {showNewAddressForm ? "Use a saved address" : "+ Use a different address"}
                </button>
              </div>
            ) : null}
            ...
            <div className="mt-4">
              ...
              {isLoadingZones ? (
                ...
                <Select
                  value={selectedZone}
                  onValueChange={(value) => { setSelectedZone(value); setSelectedSlotId(""); }}
                >
            ...
            {selectedZoneObj && (
              ...
              >
                ...
                <span className="text-sm" style={{ color: '#1A1A2E' }}>
                  Delivery fee: <strong>{formatPeso(selectedZoneObj.fee)}</strong> &middot; Est. {selectedZoneObj.estimatedTime}
                </span>
              </div>
            )}
          </div>
            ...
            <div>
              ...
              {isLoadingSlots ? (
                ...
                <p className="text-sm" style={{ color: '#6B7280' }}>
                  {selectedZone && deliveryDate ? "No slots available for this zone/date." : "Select a zone and date to see available slots."}
                </p>
              ) : (
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {displayTimeSlots.map((slot) => {
                    const isSelected = slot.id ? selectedSlotId === slot.id : deliveryTimeSlot === slot.label;
                    return (
                      <button
