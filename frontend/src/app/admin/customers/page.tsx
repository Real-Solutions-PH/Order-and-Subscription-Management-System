...
const segments: {
  ...
  { key: "all", label: "All Customers", count: (cs) => cs.length },
  {
    ...
    count: (cs) =>
      cs.filter((c) => c.status === "active" && c.planType !== "A la carte")
        .length,
  },
  {
    key: "paused",
    label: "Paused",
    count: (cs) => cs.filter((c) => c.status === "paused").length,
  },
  {
    key: "at_risk",
    label: "At Risk",
    count: (cs) => cs.filter((c) => c.status === "at_risk").length,
  },
  {
    key: "churned",
    label: "Churned",
    count: (cs) => cs.filter((c) => c.status === "churned").length,
  },
...
export default function CustomersPage() {
  ...
  const displayCustomers: Customer[] =
    usersQuery.data?.items?.map((u: UserResponse, index: number) => ({
      id: index + 1,
    ...
    switch (activeSegment) {
      case "active":
        return displayCustomers.filter(
          (c) => c.status === "active" && c.planType !== "A la carte",
        );
      case "paused":
        return displayCustomers.filter((c) => c.status === "paused");
      case "at_risk":
        return displayCustomers.filter((c) => c.status === "at_risk");
      case "churned":
        return displayCustomers.filter((c) => c.status === "churned");
      case "vip":
        return displayCustomers.filter((c) => c.isVIP);
      default:
    ...
    return [...searchFiltered].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "phone":
          ...
          break;
        case "planType":
          cmp = a.planType.localeCompare(b.planType);
          break;
        case "status":
        ...
        case "ltv":
          ...
          break;
        case "lastOrder":
          cmp = a.lastOrder.localeCompare(b.lastOrder);
          break;
        case "joinDate":
  ...
  function getAllNotes(customer: Customer) {
    ...
    return [...customer.notes.map((n) => ({ text: n, time: "" })), ...added];
  }
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 lg:hidden">
            {segments.map((seg) => (
              <button
                ...
                style={{
                  backgroundColor:
                    activeSegment === seg.key ? "#1B4332" : "#FFFFFF",
                  color: activeSegment === seg.key ? "#FFFFFF" : "#6B7280",
                  border:
                    activeSegment === seg.key ? "none" : "1px solid #E5E7EB",
                }}
              >
            ...
            <div className="space-y-1">
              {segments.map((seg) => (
                <button
                  ...
                  style={{
                    backgroundColor:
                      activeSegment === seg.key ? "#1B4332" : "transparent",
                  ...
                  <div className="p-6">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        >
                          ...
                          <p
                            ...
                            style={{ color: "#1A1A2E" }}
                          >
                            {selectedCustomer.monthsSubscribed}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: "#1B4332" }}
                      >
                        {formatPeso(selectedCustomer.ltv)}
                      </p>
                    </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              value={smsForm.phone}
              onChange={(e) =>
                setSmsForm((f) => ({ ...f, phone: e.target.value }))
              }
              className="w-full rounded-lg px-3 py-2 text-sm"
              style={{ border: "1px solid #E5E7EB", color: "#1A1A2E" }}
            />
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setSmsForm((f) => ({ ...f, message: e.target.value }))
              }
              rows={4}
              className="w-full rounded-lg px-3 py-2 text-sm"
              style={{ border: "1px solid #E5E7EB", color: "#1A1A2E" }}
