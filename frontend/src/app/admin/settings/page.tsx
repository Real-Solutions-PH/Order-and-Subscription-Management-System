...
export default function SettingsPage() {
  ...
  function handleZoneFeeChange(idx: number, fee: number) {
    setZones((prev) => (prev ?? initialZonesData).map((z, i) => (i === idx ? { ...z, fee } : z)));
  }

  function handleRemoveZone(idx: number) {
    setZones((prev) => (prev ?? initialZonesData).filter((_, i) => i !== idx));
    showToast('Zone removed');
  }

  function handleAddZone() {
    if (!newZone.name.trim()) return;
    setZones((prev) => [...(prev ?? initialZonesData), { ...newZone, id: Date.now() }]);
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setGeneral((g) => ({ ...(g ?? initialGeneral), businessName: e.target.value }))}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input
                      type="email"
                      value={general.email}
                      onChange={(e) => setGeneral((g) => ({ ...(g ?? initialGeneral), email: e.target.value }))}
                      className="w-full rounded-lg px-3 py-2 text-sm border border-border text-text-primary"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={general.phone}
                      onChange={(e) => setGeneral((g) => ({ ...(g ?? initialGeneral), phone: e.target.value }))}
                      className="w-full rounded-lg px-3 py-2 text-sm border border-border text-text-primary"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setGeneral((g) => ({ ...(g ?? initialGeneral), address: e.target.value }))}
                    rows={3}
                    className="w-full rounded-lg px-3 py-2 text-sm border border-border text-text-primary"
                  />
                </div>
