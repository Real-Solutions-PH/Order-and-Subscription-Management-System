...
const menuHistory = [
  ...
  {
    weekLabel: "Mar 17 - Mar 23",
    mealCount: 13,
    meals: [
      "Chicken Kare-Kare",
      "Grilled Fish Sinigang",
      "Garlic Butter Chicken",
      "Vegan Buddha Bowl",
      "Beef Tapa",
    ],
      ...
      try {
        ...
        setCalendar((prev) => {
          ...
          if (sourceDay && sourceDay !== targetDay) {
            updated[sourceDay] = (updated[sourceDay] || []).filter(
              (id) => id !== mealId,
            );
          }
        ...
        if (sourceDay && sourceDay !== targetDay) {
          showToast(
            `Moved ${meal?.name || "meal"} from ${sourceDay} to ${targetDay}`,
          );
        } else if (!sourceDay) {
