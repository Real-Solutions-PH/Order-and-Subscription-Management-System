...
async def _seed_admin_user(session: AsyncSession, tenant_id: uuid.UUID) -> uuid.UUID:
    if await _exists(session, User, tenant_id=tenant_id, email=settings.seed_admin_email):
        return (await session.execute(
            select(User.id).filter_by(tenant_id=tenant_id, email=settings.seed_admin_email)
        )).scalar_one()
    user_id = uuid.uuid4()
    session.add(User(
        id=user_id,
    ...
    for name, (slug, desc) in categories.items():
        if await _exists(session, ProductCategory, tenant_id=tenant_id, slug=slug):
            row = await session.execute(
                select(ProductCategory.id).filter_by(tenant_id=tenant_id, slug=slug)
            )
...
async def _seed_products_and_variants(session: AsyncSession, tenant_id: uuid.UUID) -> list[uuid.UUID]:
    ...
    for p in products:
        if await _exists(session, Product, tenant_id=tenant_id, slug=p["slug"]):
            row = await session.execute(
                select(Product.id).filter_by(tenant_id=tenant_id, slug=p["slug"])
            )
            product_id = row.scalar_one()
            row2 = await session.execute(
                select(ProductVariant.id).filter_by(product_id=product_id, is_default=True)
            )
...
async def _seed_subscription_plans(session: AsyncSession, tenant_id: uuid.UUID) -> None:
    ...
    for plan_data in plans:
        ...
        for i, tier in enumerate(plan_data["tiers"]):
            session.add(SubscriptionPlanTier(
                id=uuid.uuid4(),
                plan_id=plan_id,
                name=tier["name"],
...
async def _seed_notification_templates(session: AsyncSession, tenant_id: uuid.UUID) -> None:
    templates = [
        ...
        },
        {
            ...
            "body_template": "Hi {{first_name}}, your {{plan_name}} subscription is now active. Next billing: {{next_billing_date}}.",
        },
        {
    ...
    for t in templates:
        if await _exists(session, NotificationTemplate, tenant_id=tenant_id,
                         event_type=t["event_type"], channel=t["channel"]):
            continue
