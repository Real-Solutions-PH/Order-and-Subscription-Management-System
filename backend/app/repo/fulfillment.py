...
class AddressRepository(BaseRepository[Address]):
    ...
    async def get_by_user(self, user_id: UUID | str, tenant_id: UUID | str | None = None) -> list[Address]:
    ...
    async def get_default(self, user_id: UUID | str, tenant_id: UUID | str | None = None) -> Address | None:
...
class DeliveryZoneRepository(BaseRepository[DeliveryZone]):
    ...
    async def get_active(self, tenant_id: UUID | str | None = None) -> list[DeliveryZone]:
    ...
    async def lookup_by_postal_code(self, postal_code: str, tenant_id: UUID | str | None = None) -> DeliveryZone | None:
        ...
        for zone in zones:
            if zone.boundaries and postal_code in zone.boundaries.get("postal_codes", []):
                return zone
        return None
...
class DeliverySlotRepository(BaseRepository[DeliverySlot]):
    ...
    async def get_available(self, zone_id: UUID | str, target_date: date) -> list[DeliverySlot]:
...
class FulfillmentOrderRepository(BaseRepository[FulfillmentOrder]):
    ...
    async def get_by_order(self, order_id: UUID | str) -> FulfillmentOrder | None:
        ...
        stmt = select(FulfillmentOrder).where(FulfillmentOrder.order_id == order_id)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_by_date(self, tenant_id: UUID | str, target_date: date) -> list[FulfillmentOrder]:
        """Get all fulfillment orders scheduled for a given date."""
        stmt = select(FulfillmentOrder).where(FulfillmentOrder.scheduled_date == target_date)
        stmt = self._apply_tenant_filter(stmt, tenant_id)
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def get_by_status(self, tenant_id: UUID | str, status: FulfillmentStatus) -> Sequence[FulfillmentOrder]:
        """Get all fulfillment orders with a specific status."""
        stmt = select(FulfillmentOrder).where(FulfillmentOrder.status == status)
        stmt = self._apply_tenant_filter(stmt, tenant_id)
        result = await self.session.execute(stmt)
